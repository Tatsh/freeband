         ...
         Must support at least 1 output channel (mono) */
            if (deviceInfo->defaultSampleRate >= 44100.0f &&
                deviceInfo->maxInputChannels >=
                    1 && /* Without at least 1 input, microphone (vocals) mode cannot be utilised */
                ...
                if (i < MAX_AUDIO_DEVICES) { /* Add device to working list */
                    ...
                    audio_deviceInfoSupported[i].defaultHighOutputLatency =
                        deviceInfo->defaultHighOutputLatency;
                    audio_deviceInfoSupported[i].defaultLowOutputLatency =
                        deviceInfo->defaultLowOutputLatency;
                    audio_deviceInfoSupported[i].defaultHighInputLatency =
                        deviceInfo->defaultHighInputLatency;
                    audio_deviceInfoSupported[i].defaultLowInputLatency =
                        deviceInfo->defaultLowInputLatency;
                ...
                if (i <
                    MAX_AUDIO_DEVICES) { /* We add to non-working list, allow user to force these in options? */
                    audio_deviceInfoUnsupported[i].maxInputChannels = deviceInfo->maxInputChannels;
                    audio_deviceInfoUnsupported[i].maxOutputChannels =
                        deviceInfo->maxOutputChannels;
                    audio_deviceInfoUnsupported[i].defaultHighOutputLatency =
                        deviceInfo->defaultHighOutputLatency;
                    audio_deviceInfoUnsupported[i].defaultLowOutputLatency =
                        deviceInfo->defaultLowOutputLatency;
                    audio_deviceInfoUnsupported[i].defaultHighInputLatency =
                        deviceInfo->defaultHighInputLatency;
                    audio_deviceInfoUnsupported[i].defaultLowInputLatency =
                        deviceInfo->defaultLowInputLatency;
                    audio_deviceInfoUnsupported[i].defaultSampleRate =
                        deviceInfo->defaultSampleRate;
                }
            }
        }
