...
texture_i logo,
  selectG; /* Textures; must be called selectG thanks to Gentoo defining select randomly elsewhere this links to */
extern texture_i bg;
...
GLvoid screenMain_accept() {
  switch (screenMain_nSelection) {
    case 0:
...
#ifndef NDEBUG
      fprintf(stdout,
              "Deleted all screen mainMenu elements.\nLoading single player screenInstruments "
              "elements...\n");
#endif
      ...
      fb_screen.instruments = instrument[0].guitar =
        true; /* We set guitar to default selection, does not affect highlight */
      break;
...
bool screenMenuFooter_buffer() {
  ...
  for (i = 2; i < 4; i++)
    button_bg_X_upDown[i] =
      button_bg_X_upDown[i - 2] + graphics_scaleTextureWidth(graphics_getTextureWidth(button_bg_p),
                                                             graphics_getTextureHeight(button_bg_p),
                                                             FOOTERBG_HT + 0.03);
  ...
  for (i = 2; i < 4; i++)
    button_green_X[i] =
      button_green_X[i - 2] + graphics_scaleTextureWidth(graphics_getTextureWidth(button_green_p),
                                                         graphics_getTextureHeight(button_green_p),
                                                         BOTTOM_TEXT_HT);
  ...
  for (i = 2; i < 4; i++)
    button_red_X[i] =
      button_red_X[i - 2] + graphics_scaleTextureWidth(graphics_getTextureWidth(button_red_p),
                                                       graphics_getTextureHeight(button_red_p),
                                                       BOTTOM_TEXT_HT);
  for (i = 0; i < 4; i++)
  ...
  if ((strummer = graphics_loadTexture(strummer_p, 2)) == -1)
    fprintf(stderr, "Error loading %s texture.\n", strummer_p);
  for (i = 0; i < 2; i++)
    strummer_X[i] = graphics_centreAtX(
      0.0f,
      graphics_scaleTextureWidth(
        graphics_getTextureWidth(strummer_p), graphics_getTextureHeight(strummer_p), 0.05f));
  for (i = 2; i < 4; i++)
    strummer_X[i] =
  ...
  if ((freeSans = TTF_OpenFont(path_bold_freeSans, DEFAULT_TEXT_PT))) {
    select_G = text_load(languageStrings_screenFooter.select,
                         freeSans,
...
bool screenMain_buffer() {
  ...
  for (i = 0; i < 2; i++)
    screenMain_selectionX[i] = graphics_centreAtX(
      0.6f,
      graphics_scaleTextureWidth(
        graphics_getTextureWidth(mainSelector), graphics_getTextureHeight(mainSelector), 0.18f));
  for (i = 2; i < 4; i++)
    screenMain_selectionX[i] = screenMain_selectionX[i - 2] +
  ...
  if ((crillee = TTF_OpenFont(path_italic_crillee, DEFAULT_TEXT_PT))) {
    single = text_load(languageStrings_screenMain.single_player,
                       crillee,
...
GLvoid screenMain() {
  ...
  glPushMatrix();
  {
    ...
    glColor4f(text_SinglePlayer_hl[0],
              text_SinglePlayer_hl[1],
              text_SinglePlayer_hl[2],
              text_SinglePlayer_hl[3]);
    glBindTexture(GL_TEXTURE_2D, single);
    graphics_positionTexture(text_SinglePlayerX, text_SinglePlayerY, defVertexZ);

    glColor4f(text_Multiplayer_hl[0],
              text_Multiplayer_hl[1],
              text_Multiplayer_hl[2],
              text_Multiplayer_hl[3]);
