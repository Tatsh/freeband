...
text_i combo0, combo1, combo2, combo3, combo4, combo5, combo6, combo7, combo8,
  combo9; /* Combo numbers */
text_i score0, score1, score2, score3, score4, score5, score6, score7, score8,
  score9; /* Score numbers */
...
void screenGame_loop() {
  ...
  while (songRunning) {
    ...
    if (timing_window[0].note != DEAD_NOTE ||
        timing_window[0].note !=
          SONG_END) { /* If the note is not a 'dead' note, where no buttons are hit, process */
  ...
  if (!gamePaused) {
    ...
    glPushMatrix();
    {
      ...
      glBegin(GL_QUADS);
      {
        ...
        glTexCoord2f(NE_coord_pos, 1.0);
        glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
        glVertex3f(-3.5f, 0.5f, -TRACKWIDTH);
        glTexCoord2f(NE_coord_pos, 0.0);
        glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
        glVertex3f(-3.5f, 0.5f, TRACKWIDTH);
        glTexCoord2f(NE_coord_neg, 0.0);
        glColor4f(1.0f, 1.0f, 1.0f, 0.0f);
        glVertex3f(2.0f, 0.5f, TRACKWIDTH);
        glTexCoord2f(NE_coord_neg, 1.0);
        glColor4f(1.0f, 1.0f, 1.0f, 0.0f);
        glVertex3f(2.0f, 0.5f, -TRACKWIDTH);
      }
      glEnd();
    ...
    if (bringDownAngle < 0.0f) { /* Only load buttons, etc after has been placed down entirely */
      ...
      if (screenGame_button.r)
        ...
        glColor4f(
          buttonColour_red[0], buttonColour_red[1], buttonColour_red[2], buttonColour_red[3]);
      graphics_positionTexture(button_sizeX, button_sizeY, defVertexZ);
      glPopMatrix();
      ...
      if (screenGame_button.b)
        ...
        glColor4f(
          buttonColour_blue[0], buttonColour_blue[1], buttonColour_blue[2], buttonColour_blue[3]);
      graphics_positionTexture(button_sizeX, button_sizeY, defVertexZ);
      glPopMatrix();
