...
bool screenPause_buffer() {
    ...
    if ((freeSans = TTF_OpenFont(path_bold_freeSans, DEFAULT_TEXT_PT))) {
        ...
        width = text_scaleWidth(
            languageStrings_screenPause.change_difficulty, freeSans, PAUSE_MENU_ITEMS_HT);
        for (i = 0; i < 2; i++)
        ...
        width =
            text_scaleWidth(languageStrings_screenPause.lefty_mode, freeSans, PAUSE_MENU_ITEMS_HT);
        for (i = 0; i < 2; i++)
